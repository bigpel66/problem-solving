...
func (p *Problem) input() {
	for i := 0; i < 2; i++ {
		...
		if _, err := fmt.Fscan(p.Reader, &apple, &orange); err != nil {
			os.Exit(1)
		}
		p.Data.apple = append(p.Data.apple, apple)
		p.Data.orange = append(p.Data.orange, orange)
	}
}

func (p *Problem) logic() {
	var val = int(math.Min(float64(p.apple[0]+p.orange[1]), float64(p.apple[1]+p.orange[0])))
	p.Append(val)
}
