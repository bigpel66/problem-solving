...
func (p *Problem) logic() {
	if p.arr[0] < p.arr[1] && p.arr[1] < p.arr[2] && p.arr[2] < p.arr[3] {
		p.Append("Fish Rising")
	} else if p.arr[0] > p.arr[1] && p.arr[1] > p.arr[2] && p.arr[2] > p.arr[3] {
		p.Append("Fish Diving")
	} else if max(p.arr[0], p.arr[1:]...) == min(p.arr[0], p.arr[1:]...) {
		p.Append("Fish At Constant Depth")
	} else {
		p.Append("No Fish")
	}
}
