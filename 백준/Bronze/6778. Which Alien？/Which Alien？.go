...
func (p *Problem) input() {
	if _, err := fmt.Fscan(p.Reader, &p.antenna, &p.eyes); err != nil {
		os.Exit(1)
	}
}

func (p *Problem) logic() {
	if p.antenna >= 3 && p.eyes <= 4 {
		p.Append("TroyMartian\n")
	}
	if p.antenna <= 6 && p.eyes >= 2 {
		p.Append("VladSaturnian\n")
	}
	if p.antenna <= 2 && p.eyes <= 3 {
		p.Append("GraemeMercurian\n")
	}
}
