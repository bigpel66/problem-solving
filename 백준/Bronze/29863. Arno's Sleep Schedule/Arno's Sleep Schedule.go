...
func (p *Problem) input() {
	if _, err := fmt.Fscan(p.Reader, &p.sleep, &p.wakeup); err != nil {
		os.Exit(1)
	}
}

func (p *Problem) logic() {
	t1 := time.Date(1984, time.November, 2, p.sleep, 0, 0, 0, time.UTC)
	t2 := time.Date(1984, time.November, 3, p.wakeup, 0, 0, 0, time.UTC)
	if t1.Hour() < 5 {
