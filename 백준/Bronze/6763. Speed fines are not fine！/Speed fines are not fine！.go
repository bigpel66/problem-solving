...
func (p *Problem) input() {
	if _, err := fmt.Fscan(p.Reader, &p.limit, &p.speed); err != nil {
		os.Exit(1)
	}
}

func (p *Problem) logic() {
	if diff := p.speed - p.limit; diff <= 0 {
		p.Append("Congratulations, you are within the speed limit!")
	} else {
		var fine int
		if diff >= 1 && diff <= 20 {
			fine = 100
		...
		p.
			Append("You are speeding and your fine is $").
			Append(fine).
			Append(".\n")
	}
}
